...
from twneo.common.elements.tweet_tag_relation import TweetTagRelation
...
class TwitterQueryHandler:
    ...
    def handle_tweet_search_response(self, json_resp):
        ...
        for json_data in json_resp['data']:
            ...
            referenced_tweets = json_data['referenced_tweets'] if 'referenced_tweets' in json_data else []
            ...
            self.parse_tags(entities, tweet, tweet_tag_relations)
        ...
        while start < len(all_referenced_tweet_ids):
            ...
            for tweet_dict in ref_tweet_details['data']:
                ...
                entities = tweet_dict['entities'] if 'entities' in  tweet_dict else []
        ...
        return users, tweets, user_tweet_relations, tweet_tweet_relations,tweet_tag_relations

    def parse_tags(self, entities, tweet, tweet_tag_relations):
        tags = entities['hashtags'] if 'hashtags' in entities else []
        for tag in tags:
            tag_relation = TweetTagRelation(tweetId=tweet.tweet_id, tag=tag['tag'], relation_name="tagged")
            tweet_tag_relations.append(tag_relation)
